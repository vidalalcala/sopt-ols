...
public:
    ...
    void optimize(_UniformRandomNumberGenerator& __urng)
    {
        ...
        SeqOls Estimator ;
        Estimator.useObservations( X.rows(0,_n_predict_initial - 1 ) , Y .rows(0, _n_predict_initial -1 ) ) ;
...
private:
    ...
    int _n_predict_initial ; /*!< The number of gradient samples used to initialize the stochastic optimization */
    mat _alphas_initial ;
